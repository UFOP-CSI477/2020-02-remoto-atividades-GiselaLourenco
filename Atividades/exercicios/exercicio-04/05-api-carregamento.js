...
function carregarDistritos() {
    const cidades = document.getElementById("cidades");
    const cidade_index = cidades.selectedIndex;
    const cidade_id = cidades.options[cidade_index].value;

    if(cidade_id == ""){
        return;
    }
    
    const url_distritos = `https://servicodados.ibge.gov.br/api/v1/localidades/municipios/${cidade_id}/distritos`;
